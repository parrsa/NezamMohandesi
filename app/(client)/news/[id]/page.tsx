"use client";

import React, { useState } from "react";
import ImagesSlider from "@/app/components/imageSlider";
import Image from "next/image";
import Pic from "@/public/assets/News/Gashnvare.jpg";
import Link from "next/link";
import { useParams } from "next/navigation";
import { useGetSubCategory } from "@/app/core/services/Categories/useCategories";

export const DateIcon = () => (
  <svg
    width="16"
    height="16"
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M5.33337 3.8335C5.06004 3.8335 4.83337 3.60683 4.83337 3.3335V1.3335C4.83337 1.06016 5.06004 0.833496 5.33337 0.833496C5.60671 0.833496 5.83337 1.06016 5.83337 1.3335V3.3335C5.83337 3.60683 5.60671 3.8335 5.33337 3.8335Z"
      fill="#64748B"
    />
    <path
      d="M10.6666 3.8335C10.3933 3.8335 10.1666 3.60683 10.1666 3.3335V1.3335C10.1666 1.06016 10.3933 0.833496 10.6666 0.833496C10.94 0.833496 11.1666 1.06016 11.1666 1.3335V3.3335C11.1666 3.60683 10.94 3.8335 10.6666 3.8335Z"
      fill="#64748B"
    />
    <path
      d="M5.66667 9.66655C5.58 9.66655 5.49333 9.64656 5.41333 9.61322C5.32667 9.57989 5.26 9.53321 5.19333 9.47321C5.07333 9.34654 5 9.17988 5 8.99988C5 8.91322 5.02 8.82655 5.05333 8.74655C5.08667 8.66655 5.13333 8.59322 5.19333 8.52655C5.26 8.46655 5.32667 8.41987 5.41333 8.38654C5.65333 8.28654 5.95333 8.33989 6.14 8.52655C6.26 8.65322 6.33333 8.82655 6.33333 8.99988C6.33333 9.03988 6.32667 9.08656 6.32 9.13322C6.31333 9.17322 6.3 9.21322 6.28 9.25322C6.26667 9.29322 6.24667 9.33321 6.22 9.37321C6.2 9.40655 6.16667 9.43988 6.14 9.47321C6.01333 9.59321 5.84 9.66655 5.66667 9.66655Z"
      fill="#64748B"
    />
    <path
      d="M8.00004 9.66658C7.91337 9.66658 7.82671 9.64659 7.74671 9.61326C7.66004 9.57992 7.59337 9.53324 7.52671 9.47324C7.40671 9.34658 7.33337 9.17992 7.33337 8.99992C7.33337 8.91325 7.35338 8.82658 7.38671 8.74658C7.42004 8.66658 7.46671 8.59325 7.52671 8.52659C7.59337 8.46659 7.66004 8.41991 7.74671 8.38657C7.98671 8.27991 8.28671 8.33992 8.47337 8.52659C8.59337 8.65325 8.66671 8.82658 8.66671 8.99992C8.66671 9.03992 8.66004 9.08659 8.65337 9.13326C8.64671 9.17326 8.63337 9.21325 8.61337 9.25325C8.60004 9.29325 8.58004 9.33325 8.55338 9.37325C8.53338 9.40658 8.50004 9.43991 8.47337 9.47324C8.34671 9.59324 8.17337 9.66658 8.00004 9.66658Z"
      fill="#64748B"
    />
    <path
      d="M10.3333 9.66658C10.2466 9.66658 10.16 9.64659 10.08 9.61326C9.99329 9.57992 9.92663 9.53324 9.85996 9.47324C9.83329 9.43991 9.80662 9.40658 9.77996 9.37325C9.75329 9.33325 9.73329 9.29325 9.71996 9.25325C9.69996 9.21325 9.68663 9.17326 9.67996 9.13326C9.67329 9.08659 9.66663 9.03992 9.66663 8.99992C9.66663 8.82658 9.73996 8.65325 9.85996 8.52659C9.92663 8.46659 9.99329 8.41991 10.08 8.38657C10.3266 8.27991 10.62 8.33992 10.8066 8.52659C10.9266 8.65325 11 8.82658 11 8.99992C11 9.03992 10.9933 9.08659 10.9866 9.13326C10.98 9.17326 10.9666 9.21325 10.9466 9.25325C10.9333 9.29325 10.9133 9.33325 10.8866 9.37325C10.8666 9.40658 10.8333 9.43991 10.8066 9.47324C10.68 9.59324 10.5066 9.66658 10.3333 9.66658Z"
      fill="#64748B"
    />
    <path
      d="M5.66667 11.9998C5.58 11.9998 5.49333 11.9798 5.41333 11.9465C5.33333 11.9131 5.26 11.8665 5.19333 11.8065C5.07333 11.6798 5 11.5065 5 11.3331C5 11.2465 5.02 11.1598 5.05333 11.0798C5.08667 10.9931 5.13333 10.9198 5.19333 10.8598C5.44 10.6131 5.89333 10.6131 6.14 10.8598C6.26 10.9865 6.33333 11.1598 6.33333 11.3331C6.33333 11.5065 6.26 11.6798 6.14 11.8065C6.01333 11.9265 5.84 11.9998 5.66667 11.9998Z"
      fill="#64748B"
    />
    <path
      d="M8.00004 11.9998C7.82671 11.9998 7.65337 11.9265 7.52671 11.8065C7.40671 11.6798 7.33337 11.5065 7.33337 11.3331C7.33337 11.2465 7.35338 11.1598 7.38671 11.0798C7.42004 10.9931 7.46671 10.9198 7.52671 10.8598C7.77337 10.6131 8.22671 10.6131 8.47337 10.8598C8.53337 10.9198 8.58004 10.9931 8.61337 11.0798C8.64671 11.1598 8.66671 11.2465 8.66671 11.3331C8.66671 11.5065 8.59337 11.6798 8.47337 11.8065C8.34671 11.9265 8.17337 11.9998 8.00004 11.9998Z"
      fill="#64748B"
    />
    <path
      d="M10.3333 11.9999C10.16 11.9999 9.98663 11.9266 9.85996 11.8066C9.79996 11.7466 9.75329 11.6733 9.71996 11.5866C9.68663 11.5066 9.66663 11.4199 9.66663 11.3333C9.66663 11.2466 9.68663 11.1599 9.71996 11.0799C9.75329 10.9933 9.79996 10.9199 9.85996 10.8599C10.0133 10.7066 10.2466 10.6333 10.46 10.6799C10.5066 10.6866 10.5466 10.6999 10.5866 10.7199C10.6266 10.7333 10.6666 10.7533 10.7066 10.78C10.74 10.8 10.7733 10.8333 10.8066 10.8599C10.9266 10.9866 11 11.1599 11 11.3333C11 11.5066 10.9266 11.6799 10.8066 11.8066C10.68 11.9266 10.5066 11.9999 10.3333 11.9999Z"
      fill="#64748B"
    />
    <path
      d="M13.6667 6.56006H2.33337C2.06004 6.56006 1.83337 6.33339 1.83337 6.06006C1.83337 5.78673 2.06004 5.56006 2.33337 5.56006H13.6667C13.94 5.56006 14.1667 5.78673 14.1667 6.06006C14.1667 6.33339 13.94 6.56006 13.6667 6.56006Z"
      fill="#64748B"
    />
    <path
      d="M10.6667 15.1668H5.33333C2.9 15.1668 1.5 13.7668 1.5 11.3335V5.66683C1.5 3.2335 2.9 1.8335 5.33333 1.8335H10.6667C13.1 1.8335 14.5 3.2335 14.5 5.66683V11.3335C14.5 13.7668 13.1 15.1668 10.6667 15.1668ZM5.33333 2.8335C3.42667 2.8335 2.5 3.76016 2.5 5.66683V11.3335C2.5 13.2402 3.42667 14.1668 5.33333 14.1668H10.6667C12.5733 14.1668 13.5 13.2402 13.5 11.3335V5.66683C13.5 3.76016 12.5733 2.8335 10.6667 2.8335H5.33333Z"
      fill="#64748B"
    />
  </svg>
);

const banners = [
  {
    id: 1,
    title: "پیام تبریک به مناسبت ۷ خرداد، روز نقشه‌برداری",
    subTitle:
      "۷ خرداد، روز نقشه‌برداری، فرصتی ارزشمند برای پاسداشت جایگاه والای مهندسان نقشه‌بردار و نقش بنیادین آنان در توسعه، ایمنی، مدیریت شهری و تصمیم‌سازی‌های کلان کشور است.",
    titleClasses: "text-white text-center",
    subTitleClasses: "text-white text-center",
    iconClasses: "-translate-y-7 w-40",
    date: "1404/10/2 ",
  },
  {
    id: 2,
    title: "پیام تبریک به مناسبت ۷ خرداد، روز نقشه‌برداری",
    subTitle:
      "۷ خرداد، روز نقشه‌برداری، فرصتی ارزشمند برای پاسداشت جایگاه والای مهندسان نقشه‌بردار و نقش بنیادین آنان در توسعه، ایمنی، مدیریت شهری و تصمیم‌سازی‌های کلان کشور است.",
    titleClasses: "text-white text-center",
    subTitleClasses: "text-white text-center",
    iconClasses: "-translate-y-7 w-40",
    date: "1404/10/2 ",
  },
  {
    id: 3,
    title: "پیام تبریک به مناسبت ۷ خرداد، روز نقشه‌برداری",
    subTitle:
      "۷ خرداد، روز نقشه‌برداری، فرصتی ارزشمند برای پاسداشت جایگاه والای مهندسان نقشه‌بردار و نقش بنیادین آنان در توسعه، ایمنی، مدیریت شهری و تصمیم‌سازی‌های کلان کشور است.",
    titleClasses: "text-white text-center",
    subTitleClasses: "text-white text-center",
    iconClasses: "-translate-y-7 w-40",
    date: "1404/10/2 ",
  },
];

const newsCategory = [
  {
    id: 1,
    name: "اخبار",
    news: [
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 11,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 12,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 13,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 14,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 15,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 16,
      },
    ],
  },
  {
    id: 2,
    name: "اطلاعیه‌ها",
    news: [
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 17,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 18,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 19,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 20,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 21,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 22,
      },
    ],
  },
  {
    id: 3,
    name: "فراخوان",
    news: [
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 23,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 24,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 25,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 26,
      },
      {
        title: "نشست تخصصی فناوری‌های دیجیتال و هوش مصنوعی",
        content:
          "سازمان نظام مهندسی ساختمان استان تهران، در راستای توسعه فناوری‌های نوین، هوشمندسازی صنعت ساختمان و بهره‌گیری هدفمند از ظرفیت‌های هوش ",
        image: Pic,
        creationDate: "1404/10/2",
        id: 27,
      },
    ],
  },
];

export default function NewsPage() {
  const { id } = useParams();

  const [selectedCategory, setSelectedCategory] = useState(1);

  const { data, isLoading, error, refetch } = useGetSubCategory(String(id));

  return (
    <div className="p-5">
      <div className="flex flex-col p-3">
        <div className="flex items-start justify-center p-3 gap-8 bg-blue-100 rounded-t-2xl">
          <div className="bg-white rounded-2xl w-64 h-48 p-3">
            <Image src={Pic} alt="pic" className="h-full" />
          </div>
          <div className="max-w-4xl">
            <ImagesSlider banners={banners} autoPlay />
          </div>
        </div>
        <div className="bg-white flex flex-col gap-3 p-3 rounded-b-2xl">
          <div className="flex items-center justify-center gap-2 mb-2">
            {data?.map((category: any) => (
              <div
                onClick={() => setSelectedCategory(category.id)}
                key={category.id}
                className={`w-28 p-1 rounded-full text-center cursor-pointer ${selectedCategory === category.id ? "bg-[#2563EB1A] text-gray-800" : "bg-white text-gray-800 border border-gray-800"}`}
              >
                <p className="text-gray-800 text-[12px] font-bold text-nowrap">
                  {category.name}
                </p>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-2">
            {newsCategory.map((categories, index) => (
              <React.Fragment key={index}>
                {selectedCategory === categories.id &&
                  categories.news.map((item, itemIndex) => (
                    <Link
                      href={`/news/${item.id}`}
                      key={itemIndex}
                      className="flex items-center gap-3 p-3 border border-gray-100 shadow-md rounded-xl"
                    >
                      <Image
                        src={item.image}
                        alt={item.title}
                        width={150}
                        height={100}
                        className="rounded-xl"
                      />
                      <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-2">
                          <DateIcon />
                          <p className="text-[13px] text-gray-600">
                            {item.creationDate}
                          </p>
                        </div>
                        <p className="text-sm font-bold text-gray-800">
                          {item.title}
                        </p>
                        <p className="text-sm text-gray-600 leading-7">
                          {item.content}
                        </p>
                      </div>
                    </Link>
                  ))}
              </React.Fragment>
            ))}
          </div>
        </div>
      </div>
    </div>
  );
}
